...
restaurants.loc[restaurants['length_of_stay'].str.match('(long|short)') & restaurants["area"] == 'cventer']
...
def find_all_restaurants(df, preferences):
    if(len(preferences) == 3):
        if(preferences[0] == 'any'):
            result = df.loc[ (df["area"] == preferences[1]) & (df["food"] == preferences[2])]            
        else:    
        ...
        if(preferences[3] == 'romantic'):
            result = df.loc[ df['pricerange'].str.match(pricerange) & (df["area"] == preferences[1]) & (df["food"] == preferences[2]) & (df['length_of_stay'] == 'long') & (df['crowdedness'] == 'not busy')]
            
        if(preferences[3] == 'children'):
            result = df.loc[df['pricerange'].str.match(pricerange) & (df["area"] == preferences[1]) & (df["food"] == preferences[2]) & (df['length_of_stay'] == 'short')]
            
        if(preferences[3] == 'assigned seats'):
            result = df.loc[df['pricerange'].str.match(pricerange) & (df["area"] == preferences[1]) & (df["food"] == preferences[2]) & (df['crowdedness'] == 'busy')] 
            
        if(preferences[3] == 'touristic'):
            result = df.loc[df['pricerange'].str.match(pricerange) & (df["area"] == preferences[1]) & (df["food"] == preferences[2]) & (df['crowdedness'] == 'busy') & (df["food"] != 'romanian') & df['food_quality'] == 'good food']
...
find_all_restaurants(restaurants,['any', 'centre', 'indian','assigned seats'])
...
def toString_recommended_restaurant(index):
    ...
    if(len(globals()['remaining_restaurants']) > 0) and (globals()['extra_requirement'] == ''):
