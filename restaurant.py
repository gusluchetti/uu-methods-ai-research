...
def find_all_restaurants(df, preferences):
    if(len(preferences) == 3):
        ...
        result = df.loc[(df["pricerange"] == preferences[0]) & (df["area"] == preferences[1]) & (df["food"] == preferences[2]) & (df["length_of_stay"] == preferences[3]) & (df["crowdedness"] == preferences[4]) & (df["food_quality"] == preferences[5]) ]
        result.reset_index(drop=True,inplace=True)
        globals()['remaining_restaurants'] = result
        return result
    else:
...
find_all_restaurants(pd.read_csv(r'C:\Users\niels\restaurant_info(1).csv'),['moderate', 'centre', 'british'])
